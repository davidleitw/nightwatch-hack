...
REPAIR_PROMPT = """
The operator enabled local demo-fault repair for this session. First read get_graph, then get_demo_faults early because fault leases are short. If a fault is active, explain the intended remediation in relation to the previously established root cause (or explicitly tentative hypothesis) and deactivate_demo_fault promptly using exactly the observed fault_id and started_at; do not spend the lease on historical queries first. The operator authorizes this bounded demo action without another approval. Never inject faults or invent other runtime operations.
