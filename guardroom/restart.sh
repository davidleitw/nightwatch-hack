...
for ((attempt=0; attempt<50; attempt++)); do
  ...
  if curl --fail --silent --max-time 1 "http://127.0.0.1:$port/api/graph?state=normal" >/dev/null; then
    printf 'Server ready (PID %s)\nNormal: http://127.0.0.1:%s/api/graph?state=normal\nProblem: http://127.0.0.1:%s/api/graph?state=problem\nLog: %s\n' "$server_pid" "$port" "$port" "$log_file"
    exit 0
  fi
  sleep 0.2
done
